...
export default {
  data () {
    return {
      ...
      optionGolongan: [
        'Golongan Beta Lactams',
        'Golongan Cell Membrane',
        'Golongan Folate Synthesis'
      ],
      image: null
    }
  },
  ...
  methods: {
    getData () {
      this.$axios.get(`obat/getbyid/${this.$route.params.id}`)
        .then(res => {
          if (res.data.sukses) {
    ...
    onSubmit () {
      ...
      this.$axios.put(`obat/edit/${this.$route.params.id}`, formData)
        .then(res => {
          if (res.data.sukses) {
