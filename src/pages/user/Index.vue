<template>
    <q-page padding>
        ...
        <div class="col-md-3 col-xs-12" v-for="(obat, i) in data" :key="i">
            <q-card>
            <q-img
              :src="`${$baseImageURL}/${obat.image}`"
              :ratio="16/9"
              />
            ...
            <div @click="obat.expanded = !obat.expanded" class="text-caption text-grey cursor-pointer">
              Tampilkan Deskripsi
            </div>
            <q-slide-transition>
              <div v-show="obat.expanded">
        ...
        <q-card-actions>
            <q-btn unelevated @click="openDetail(obat)" class="full-width" color="primary">
            Order Now
            </q-btn>
        </q-card-actions>
        </q-card>
      ...
      <q-card style="width: 500px">
        ...
        <q-card-section style="max-height: 50vh;" class="scroll">
            {{ activeData.namaObat }} - Rp {{ activeData.harga }},-
  ...
  methods: {
    getData () {
      this.$axios.get('obat/getall')
        .then(res => {
          if (res.data.sukses) {
            this.data = res.data.data.map(obat => {
              return Object.assign(obat, {
                expanded: false
              })
            })
          } else {
